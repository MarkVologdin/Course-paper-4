...
#define LATITUDE 55 // широта москвы в градусах 
#define LONGITUDE 37 // долгота москвы в градусах 
#define ALTITUDE 30 // высота москвы в метрах
...
double gravity_helmert(double latitude, double height) {
    /*
    ...
    double lat_rad = D2R(latitude); // Перевод широты в радианы
...
typedef struct {
    ...
    double latitude;   // Широта (λ)
    double longitude;  // Долгота (φ)
    double altitude;   // Высота (h)
    double heading;    // Курс (ψ)
    double roll;       // Крен (θ)
    double pitch;      // Тангаж (γ)
...
void initialize_data(DataPoint *data, int size, double start_heading_value, double stop_heading_value) {
    double g_0 = gravity_helmert(LATITUDE, ALTITUDE);
    ...
    for (int i = 0; i < size; i++) {
        ...
        data[i].heading = sigmoid(data[i].timestamp, start_heading_value, stop_heading_value, TIME_START, TIME_STOP);
        data[i].roll = 0;
        data[i].pitch = 0;
...
// Функция для генерации белого шума (нормального распределения)
void generate_white_noise(DataPoint *data, int size) {
    for (int i = 0; i < size; i++) {
        data[i].value = data[i].value + ((double)rand() / RAND_MAX) * 2.0 - 1.0; // Шум в диапазоне [-1, 1]
    }
}
...
int main() {
    ...
    for (int exp = 0; exp < NUM_EXPERIMENTS; exp++) {
        initialize_data(data, DATA_POINTS);
